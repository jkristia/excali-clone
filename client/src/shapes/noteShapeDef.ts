...
export class NoteShapeDef implements ShapeDefinition<NoteShape> {
    ...
    /** Distance from a note's top edge to the first line of text, vertically centering the
     *  wrapped text block within `height` (floored at {@link NOTE_PADDING} so it never rides
     *  the top edge). Shared by rendering and the editor's padding so the two line up. */
    ...
    public draw(ctx: CanvasRenderingContext2D, shape: NoteShape): void {
        ...
        CanvasDraw.wrapText(
            ...
            fs * NOTE_LINE_HEIGHT, shape.textAlign ?? 'left',
        );
    }
}
