...
function note(id: string): Shape {
    return { id, type: 'note', x: 0, y: 0, z: 0, w: 10, h: 10, fill: '#fff', text: '', createdBy: 'x' };
}
...
describe('panelFlags', () => {
    it('empty selection: reads the armed tool\'s panelCapabilities', () => {
        expect(panelFlags(toolRegistry, shapeRegistry, 'note', [])).toEqual({ stroke: false, fill: false, width: false, ends: false, note: true });
    });
    ...
    it('with selection: unions capabilities across all selected shapes', () => {
        ...
        expect(flags).toEqual({ stroke: true, fill: true, width: true, ends: false, note: true });
    });
});
